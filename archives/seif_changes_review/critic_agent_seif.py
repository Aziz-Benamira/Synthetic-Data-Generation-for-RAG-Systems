...
def numbers_are_similar(num1_str: str, num2_str: str, tolerance: float = 0.02) -> bool:
    """Check if two numbers are similar (allow ±2% tolerance for rounding)."""
    try:
...
def is_why_how_question(question: str) -> bool:
    """Check if question is CLEARLY a why/how question requiring deep explanation."""
    question_lower = question.lower().strip()
    # Only strict for clear explanatory questions
    return (question_lower.startswith('pourquoi') or 
            question_lower.startswith('expliquez pourquoi') or
            question_lower.startswith('comment se fait-il') or
            question_lower.startswith('pour quelle raison') or
            question_lower.startswith('why') or
            question_lower.startswith('explain why') or
            question_lower.startswith('how come'))
...
class CriticAgent:
    ...
    def _apply_hard_rules(
        ...
        TUNED VERSION - More lenient thresholds for production.
        ...
        # TUNED: Allow ±2% tolerance for rounding errors
        answer_numbers = extract_numbers(answer)
        chunk_numbers = extract_numbers(chunk_content)
        
        # Check if any answer number is truly unexpected (not similar to any chunk number)
        truly_unexpected = []
        for ans_num in answer_numbers:
            found_similar = False
            for chunk_num in chunk_numbers:
                if numbers_are_similar(ans_num, chunk_num, tolerance=0.02):
                    found_similar = True
                    break
            if not found_similar:
                truly_unexpected.append(ans_num)
        
        if truly_unexpected:
            logging.info(f"[HARD RULE 1] Numbers rule triggered: {truly_unexpected} not in chunk")
            criteria_evaluations["anchoring"] = CriterionEvaluation(
                criterion="anchoring",
                result=CriterionResult.FAIL,
                score=0.0,
                explanation=f"🔴 HARD RULE 1: Nombres dans la réponse absents du chunk: {truly_unexpected}",
                evidence=[]
            )
        
        # RULE 2: Why/How questions need causal markers
        # TUNED: Only for CLEAR explanatory questions, check answer too
        if is_why_how_question(qa_pair.question):
            if not has_causal_markers(chunk_content) and not has_causal_markers(answer):
                ...
                criteria_evaluations["local_answerability"] = CriterionEvaluation(
                    ...
                    result=CriterionResult.FAIL,
                    score=0.2,  # Less harsh
                    explanation="🔴 HARD RULE 2: Question explicative sans marqueurs de causalité dans le chunk ou la réponse",
                    evidence=[]
                )
        
        # RULE 3: Short answers for complex questions → COMPLETENESS FAIL
        # TUNED: 15+ words question, <40 chars answer (was 10/50)
        question_word_count = len(qa_pair.question.split())
        answer_char_count = len(answer)
        
        if question_word_count > 15 and answer_char_count < 40:
            ...
            criteria_evaluations["completeness"] = CriterionEvaluation(
                ...
                result=CriterionResult.FAIL,
                score=0.4,  # Less harsh
                explanation=f"🔴 HARD RULE 3: Réponse très courte ({answer_char_count} car) pour question complexe ({question_word_count} mots)",
                evidence=[]
            )
        
        # RULE 4: Answer is just question rephrased → COMPLETENESS FAIL
        # TUNED: 80% overlap threshold (was 70%), check answer length >10 words
        question_words = set(qa_pair.question.lower().split())
        answer_words = set(answer.lower().split())
        # Remove common words
        common_words = {'le', 'la', 'les', 'un', 'une', 'des', 'de', 'du', 'd', 'à', 'au', 'aux', 'en', 'et', 'ou', 'est', 'sont', 'a', 'ont', 
                       'que', 'qui', 'quoi', 'comment', 'pourquoi', 'where', 'what', 'when', 'which', 'the', 'a', 'an', 'is', 'are', 'of'}
        question_words -= common_words
        answer_words -= common_words
        
        if len(question_words) > 0 and len(answer.split()) > 10:
            overlap = len(question_words & answer_words) / len(question_words)
            if overlap > 0.80 and len(answer.split()) < 20:
        ...
        # TUNED: More markers, only in question
        oral_markers = ['truc', 'machin', 'chose', "c'est quoi", 'ça', 'y a', 'genre', 'genre de', 'en fait', 'du coup', 
                       'stuff', 'thing', 'kinda', 'sorta', 'like']
        ...
        if detected_markers:
            ...
            criteria_evaluations["clarity"] = CriterionEvaluation(
                ...
                result=CriterionResult.FAIL,
                score=0.2,  # Less harsh
                explanation=f"🔴 HARD RULE 5: Langage oral/familier dans la question: {detected_markers}",
                evidence=[]
            )
        
        # RULE 6 (NEW): Vague pronouns without clear referents → CLARITY WARNING
        # Check for "il", "elle", "cela", "this", "it" at start of answer
        answer_start = answer[:100].lower()
        vague_starts = ['il ', 'elle ', 'cela ', 'ça ', 'this ', 'it ', 'that ']
        if any(answer_start.startswith(v) for v in vague_starts):
            # Only warn if current clarity score is borderline
            if criteria_evaluations["clarity"].score < 0.85:
                logging.info(f"[HARD RULE 6] Vague pronoun at answer start")
                current_score = criteria_evaluations["clarity"].score
                criteria_evaluations["clarity"] = CriterionEvaluation(
                    criterion="clarity",
                    result=CriterionResult.FAIL if current_score < 0.7 else CriterionResult.PASS,
                    score=max(0.3, current_score - 0.15),  # Penalty
                    explanation=f"⚠️ HARD RULE 6: Réponse commence par pronom vague sans référent clair. {criteria_evaluations['clarity'].explanation}",
                    evidence=[]
                )
